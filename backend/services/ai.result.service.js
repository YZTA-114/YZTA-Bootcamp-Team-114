// (Burada importlar veya doğrudan class AIService başlamalı)
...
class AIService {
  ...
  async generateQuizReport(reportData) {
    const prompt = `You are an educational AI assistant designed for visually impaired learners.  
After the student completes a quiz, generate a feedback report that is compatible with screen readers. The report must be in plain, linear text — no lists, tables, or visuals.  
The content must be short, direct, and easy to understand.
...
2. Suggestion  
- Briefly state what the student needs to study more  
- Mention one strong topic and one weak topic
...
- Do not use lists, tables, or any visual content  
- The report must not exceed 250 words  
- Format everything as continuous plain text, so that it can be read easily by screen readers  

Begin with:  
“Quiz Sonlandırıldı Sonuçlar Hazır”  
Then write the report as instructed and translate in Turkish.
...
/**
 * Belirli bir quiz için tüm katılım ve yanıt verilerini toplayıp AI raporu oluşturur
 * @param {string} quizId - Raporu oluşturulacak quizin ID'si
 * @returns {Promise<string>} - AI tarafından oluşturulan özet rapor
 */
async function generateQuizReportForQuiz(quizId) {
  const QuizTake = require('../models/quiz/QuizTake');
  const QuizResponse = require('../models/quiz/QuizResponse');
  // 1. Tüm QuizTake'leri bul
  const takes = await QuizTake.find({ quiz: quizId }).populate('user', 'email');
  // 2. Her take için yanıtları topla
  const takesWithResponses = await Promise.all(
    takes.map(async (take) => {
      const responses = await QuizResponse.find({ quizTake: take._id })
        .populate('quizQuestion', 'content')
        .populate('answerOption', 'content isCorrect');
      return {
        take,
        responses,
      };
  ...
  // 3. AI rapor fonksiyonuna gönderilecek veri
  const reportData = {
    takes: takesWithResponses,
  };
  // 4. AI raporunu oluştur
  const report = await aiService.generateQuizReport(reportData);
  return report;
}

module.exports = aiService;
module.exports.generateQuizReportForQuiz = generateQuizReportForQuiz;
